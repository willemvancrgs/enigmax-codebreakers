...
def encrypt(plaintext: str, rail_count: int = 3,
            offset: int = 0
            ) -> dict[int, str]:
...
def decrypt(ciphertext: str, rail_count: int = 3,
            offset: int = 0, order: list[int] | None = None
            ) -> str:
    ...
    # Reorders the count of charecters in each line
    counts = [counts.copy()[i] for i in order]
    # Flips the key-value pairs for the order
    order = [order.index(i) for i in range(rail_count)]
    ...
    for _ in range(text_length):
        plaintext.append(ciphertext[rail_positions[order[rail]]
                                    + rail_indices[order[rail]]])
...
def solve(ciphertext: str, rail_count: int = 3,
          offset: int = 0, order: list[int] | None = None):
